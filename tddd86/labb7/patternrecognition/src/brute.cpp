...
//        }
    ...
    for (int i = 0; i < N-1; i++)
    {
        map<double, vector<Point>> sortedLines;
        for (int j = i+1; j < N; j++)
        {
            slopeVal = points.at(i).slopeTo(points.at(j));

            sortedLines[slopeVal].push_back(points.at(j));
        }

        for(const auto& x : sortedLines)
        {
            if(x.second.size() >= 3)
            {
                render_line(scene, points.at(i), x.second.back());
                a.processEvents(); // show rendered line
            }
        }
    }
