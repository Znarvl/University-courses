...
void render_all_lines(void)
{
  ...
  int i,j,k,l;         
	...
	int BUFFER_AMMOUNT = 8;
	int BUFFER_XSIZE = FB_XSIZE / BUFFER_AMMOUNT;
	k = 0;
	
	while(k < BUFFER_AMMOUNT){
	
		// Initial solution: Copy pixels from left to right, line by line
		y = 0;   // start at the top of the buffer (1st line)
		do {
			x = BUFFER_XSIZE * k;   // start to the left of the line
			do {
				...
				*/
				...
				x = x + 1;
			} while (x < BUFFER_XSIZE * (k + 1));    // Do this until line completed
			y = y + 1;
